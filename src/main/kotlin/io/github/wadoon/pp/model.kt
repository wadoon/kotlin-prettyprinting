...
value class Requirement(val value: Int) {
    ...
    operator fun plus(y: Requirement) = if (isInfinity || y.isInfinity) {
        infinity
    } else {
        Requirement(value + y.value)
    }
...
sealed class Document {
     ...
     * not check, that [Document.FancyString] do not contain a newline character.
     */
    data class FancyString(val s: kotlin.String, val ofs: Int, val len: Int, val apparentLength: Int) : Document() {
        constructor(s: kotlin.String, apparentLength: Int) : this(s, 0, s.length, apparentLength)
    }

    /** [Blank] is a document that consists of [len] blank characters. */
    data class Blank(val len: Int) : Document()

    /** When in flattening mode, [IfFlat] turns into the document
     * [doc1]. When not in flattening mode, it turns into the document [doc2]. */
